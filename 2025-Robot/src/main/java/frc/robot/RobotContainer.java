...
public class RobotContainer {
        ...
        for (int i = 0; i < Constants.Autonomous.paths.length; i++) {
            try {
                autoFiles[i] = new File(
                        Filesystem.getDeployDirectory().getPath() + "/" + Constants.Autonomous.paths[i]);
    ...
    private void configureBindings() {
        ...
        OI.driverA.whileTrue(new ConditionalCommand(new InstantCommand(),
                new SetRobotStateSimple(superstructure, SuperState.RUN_CLIMB_BACK), () -> (!manualMode)));
        ...
        OI.driverY.whileTrue(new ConditionalCommand(
                new ConditionalCommand(new SetRobotStateSimpleOnce(superstructure, SuperState.DEPLOY_CLIMBER),
                        new SetRobotStateSimpleOnce(superstructure, SuperState.CLIMB), () -> (!yPressed)),
                new SetClimberPivotTorque(climber, -80, 1.0), () -> (!manualMode)));
                        ...
                        new ConditionalCommand(
                                new ConditionalCommand(
                                        ...
                                        new SetRobotStateComplicated(superstructure,
                                                SuperState.AUTO_PROCESSOR, SuperState.DEFAULT),
                                        () -> manualMode),
